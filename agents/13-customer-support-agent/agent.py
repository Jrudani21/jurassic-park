...
def generate_response(state: SupportState) -> SupportState:
    _cfg = shared_llm.TIERS["flash"]
    llm = ChatOpenAI(model=_cfg["model"], temperature=0.2, base_url=_cfg["base_url"], api_key=_cfg["api_key"])
...
def main():
    ...
    parser.add_argument("--query", help="Single question (omit for interactive mode)")
    ...
    if args.query:
        state = {"messages": [], "user_input": args.query, "retrieved_context": "", "response": "", "escalate": False}
        state["messages"].append(HumanMessage(content=args.query))
